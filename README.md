...
سامانهٔ تحت وب برای مدیریت **چند حساب WhatsApp** (هر شماره یک اکانت) و ارسال پیام‌های **مجاز** به گروه‌هایی که همان حساب عضو آن‌هاست و اجازهٔ ارسال تبلیغات در آن‌ها را دارد. هر اکانت کمپین خودش را جلو می‌برد؛ روی یک شماره دو کمپین همزمان اجرا نمی‌شود.
...
- اتصال چند اکانت واتساپ با QR جدا (هر شماره یک نشست روی دیسک)
- وضعیت‌ها: Disconnected, Connecting, QR Required, Connected, Reconnecting, Authentication Failed
- همگام‌سازی گروه‌های عضو همان اکانت فعال، جستجو، فیلتر، انتخاب تکی/چندتایی
- ویزارد کمپین ۸ مرحله‌ای + پیش‌نمایش شبیه واتساپ + تأیید نهایی؛ ارسال فقط از اکانت فعال
...
- نقش Admin / Operator، اتصال اپراتور به یک اکانت واتساپ، تم تاریک/روشن
- **چند اکانت واتساپ:** از صفحهٔ واتساپ اکانت جدید بسازید، QR همان شماره را اسکن کنید، و از منوی کناری اکانت فعال را عوض کنید. اپراتور فقط اکانت تخصیص‌داده‌شده را می‌بیند. کمپین فقط به گروه‌های عضو همان شماره ارسال می‌شود.
...
اگر دکمهٔ **اسکن همه گروه‌ها همین الان** را نمی‌بینید، UI قدیمی روی سرور مانده است. `frontend/dist` الان داخل گیت است؛ یک بار به‌روز کنید و صفحه را سخت‌رفرش کنید:
...
بعد در مرورگر Ctrl+Shift+R. سلامت UI: `curl -s http://127.0.0.1:9454/health` باید `"uiScanShare":true` باشد.
...
5. در **گروه‌های کشف‌شده** دکمهٔ **اسکن همه گروه‌ها همین الان** پیام‌ها و توضیح گروه‌های عضو را برای لینک `chat.whatsapp.com` می‌گردد، لینک معتبر و هنوز عضو‌نشده را پیشنهاد می‌کند. «عضو شو» لینک را در واتساپ باز می‌کند (Join خودکار نیست). همهٔ لینک‌های پیدا‌شده (مثلاً ۴۰۰ تا) را می‌توان یکجا کپی یا به‌صورت فایل متنی دانلود کرد. ارسال واتساپ با تأیید صریح فقط برای **یک** مخاطب است؛ چون واتساپ حدود ۱۸۰–۲۰۰ لینک در هر پیام را قبول می‌کند، لیست بلند در چند پیام پشت‌سرهم برای همان نفر می‌رود (نه ارسال انبوه به افراد مختلف).
